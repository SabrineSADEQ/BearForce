//package fr.isika.cda.javaee.utils;
//
//import javax.enterprise.context.RequestScoped;
//import javax.faces.application.FacesMessage;
//import javax.faces.context.FacesContext;
//import javax.inject.Inject;
//import javax.inject.Named;
//
//import org.primefaces.PrimeFaces;
//import org.primefaces.event.FileUploadEvent;
//import org.primefaces.event.FilesUploadEvent;
//import org.primefaces.model.file.UploadedFile;
//import org.primefaces.model.file.UploadedFiles;
//import org.primefaces.util.EscapeUtils;
//
//import fr.isika.cda.javaee.dao.ImageDao;
//import fr.isika.cda.javaee.dao.SpaceDao;
//import fr.isika.cda.javaee.presentation.viewmodel.ImageViewModel;
//
//@Named
//@RequestScoped
//public class FileUploadViewController {
//
//    private UploadedFile file;
//    private UploadedFiles files;
//    private String dropZoneText = "Drop zone p:inputTextarea demo.";
//    
//    @Inject
//    ImageDao imageDao = new ImageDao();
//    
//
//    public void upload() {
//        if (file != null) {
//            FacesMessage message = new FacesMessage("Le fichier ", file.getFileName() + " a été téléchargé avec succès");
//            FacesContext.getCurrentInstance().addMessage(null, message);
//        }
//    }
//
//    public void uploadMultiple() {
//        if (files != null) {
//            for (UploadedFile f : files.getFiles()) {
//                FacesMessage message = new FacesMessage("Le fichier ", file.getFileName() + " a été téléchargé avec succès");
//                FacesContext.getCurrentInstance().addMessage(null, message);
//            }
//        }
//    }
//
//    public void handleFileUpload(FileUploadEvent event) {
//        FacesMessage message = new FacesMessage("Le fichier ", event.getFile().getFileName() + " a été téléchargé avec succès");
//        byte[] image = event.getFile().getContent();
//        ImageViewModel imageViewModel = new ImageViewModel();
//        imageViewModel.setGymLogo(image);
//        imageDao.persistImage(imageViewModel);
//        FacesContext.getCurrentInstance().addMessage(null, message);
//    }
//
//    public void handleFileUploadTextarea(FileUploadEvent event) {
//        String jsVal = "PF('textarea').jq.val";
//        String fileName = EscapeUtils.forJavaScript(event.getFile().getFileName());
//        PrimeFaces.current().executeScript(jsVal + "(" + jsVal + "() + '\\n\\n" + fileName + " téléchargé.')");
//    }
//
//    public void handleFilesUpload(FilesUploadEvent event) {
//        for (UploadedFile f : event.getFiles().getFiles()) {
//            FacesMessage message = new FacesMessage("Le fichier ", f.getFileName() + " a été téléchargé avec succès");
//            FacesContext.getCurrentInstance().addMessage(null, message);
//        }
//    }
//
//    public UploadedFile getFile() {
//        return file;
//    }
//
//    public void setFile(UploadedFile file) {
//        this.file = file;
//    }
//
//    public UploadedFiles getFiles() {
//        return files;
//    }
//
//    public void setFiles(UploadedFiles files) {
//        this.files = files;
//    }
//
//    public String getDropZoneText() {
//        return dropZoneText;
//    }
//
//    public void setDropZoneText(String dropZoneText) {
//        this.dropZoneText = dropZoneText;
//    }
//
//}
//
//  
